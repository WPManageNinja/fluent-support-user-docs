...
const SITE_URL = 'https://docs.fluentsupport.com'
const FOOTER_TEXT = 'docs.fluentsupport.com'
const DEFAULT_TITLE = 'Fluent Support Documentation'
...
const LOGO = {
  path: 'docs/public/images/brand/fluentsupport_secondary_logo.png',
  width: 360
}
...
// This repo's `rewrites` ('docs/:slug' and 'docs/:section/:slug') serves every page under
// docs/ (one or two levels deep) at its bare slug; anything else keeps its real path.
function servedPath(rel) {
  const parts = rel.split('/')
  return parts[0] === 'docs' && (parts.length === 2 || parts.length === 3) ? parts[parts.length - 1] : rel
}
